...
class ParameterServer:
    def __init__(
        ...
        self.logger = Logger(job_name=job_name, file_path=f"./training_logs/{model_name}_ps{ps_id}.log").logger
        ...
        # self.psup = psutil.Process()
        # self.bw_nic = "eth0"
        # self.sent0 = psutil.net_io_counters(pernic=True)[self.bw_nic].bytes_sent
        # self.recv0 = psutil.net_io_counters(pernic=True)[self.bw_nic].bytes_recv
            ...
            if sync_mode == 1 or self.sync_worker_count >= self.worker_num:
                ...
                if sync_mode == 0:
                    for i in range(self.worker_num):
                        ...
                        self.logger.info(f"wrk {i} | in_comm | {self.in_cm_t[i]:.2f} ms")
                        self.logger.info(f"wrk {i} | out_comm | {self.out_cm_t[i]:.2f} ms")
                else:
                    self.ps_cp_t[worker_id] = ps_cp_t
                    self.logger.info(f"wrk {worker_id} | in_comm | {self.in_cm_t[worker_id]:.2f} ms")
                    self.logger.info(f"wrk {worker_id} | out_comm | {self.out_cm_t[worker_id]:.2f} ms")

                # cpu_percent = self.psup.cpu_percent()
                # self.logger.info(f"cpu% {cpu_percent:.3f}")

                # sent1 = psutil.net_io_counters(pernic=True)[self.bw_nic].bytes_sent
                # recv1 = psutil.net_io_counters(pernic=True)[self.bw_nic].bytes_recv
                # time1 = time.time()
                # time_diff = time1 - self.time0
                # bw_sent = (sent1 - self.sent0) / 1024 / 1024 / time_diff
                # bw_recv = (recv1 - self.recv0) / 1024 / 1024 / time_diff
                # self.logger.info(f"in_bw out_bw {bw_recv:.4f} {bw_sent:.4f}")
                # self.sent0 = sent1
                # self.recv0 = recv1
                # self.time0 = time1
                ...
                if not self.delayed:
                    if time.time() - self.start_time > 130:
                        self.delayed = True
                        self.logger.info("delay_wrk")
                        time.sleep(1)
                else:
                    time.sleep(1)

                self.logger.info("ps sending param")
    ...
    def run_worker(self):
        ...
        for epoch_idx in range(self.epoch_num):
            ...
            for iter_idx, x_pack in enumerator:
                ...
                self.logger.info(f"wrk_cp_t | {wrk_cp_t:.2f} ms")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--do_switch", type=int, default=1)
