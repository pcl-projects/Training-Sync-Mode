...
job_launcher_addr = "10.153.5.23"
res_manager_addr = job_launcher_addr
...
class ParameterServer:
    def __init__(
        ...
        self.wrk_server_ids = wrk_server_ids
        self.ps_server_id = ps_server_id
        ...
        # res_dist
        self.is_wrk_delayed = False
        self.throttled_delay = -1  # -1 means no throttled delay. >=0 means delay by throttled res
        self.use_current_iter_times_to_delay_wrk = True
        self.curr_iter_times = {}
        for i in range(worker_num):
            self.curr_iter_times[i] = 0.0
        self.res_mngr_connm = znet.SocketMsger.tcp_connect(res_manager_addr, 48848)
        self.res_mngr_connm_lis = znet.SocketMsger.tcp_connect(res_manager_addr, 48848)
        self.res_mngr_connm.send((job_name, model_name))
        self.res_mngr_connm_lis.send((job_name, model_name))
        self.res_mngr_connm_lis.send(("EST_RCONN", None))
        threading.Thread(target=self.res_mngr_lis_thread).start()
        the_data = {"ps": int(ps_server_id)}
        for i in range(worker_num):
            the_data[f"wrk{i}"] = int(wrk_server_ids[i])
        self.res_mngr_connm.send(("START_JOB", the_data))

        # combined_strg_pred
        self.num_past_res = 25
        self.num_strg_pred_iter = 30
        self.ava_res_records = []
        for _ in range(worker_num):
            res_ = [[0.0, 0.0] for _ in range(self.num_past_res)]
            self.ava_res_records.append(res_)
        self.mode_determine_remained_counts = [0 for _ in range(worker_num)]
        # self.strg_pred_remained_counts = [0 for _ in range(worker_num)]
        self.lstm_regr_m = lrm.LstmRegr()
        self.lstm_regr_m = torch.load("./trained_models/lstm_regr_m.pth")
        self.strg_pred_iterts = None
        self.strg_tags = []
        for _ in range(worker_num):
            arr = [False for _ in range(self.num_strg_pred_iter)]
            self.strg_tags.append(arr)
        # self.strg_exist_tags = [False for _ in range(self.num_strg_pred_iter)]
        self.strg_tag_indices = [0 for _ in range(self.worker_num)]
        self.curr_strg_state = False
        self.mode_determ_m = lrm.LstmRegr()
        self.mode_determ_m = torch.load("./trained_models/mode_determ_m.pth")
        self.sync_hybrid_mode = self.worker_num
        self.sync_hybrid_mode_groups = None
        self.group_gradients = {}

    # res_dict
    def res_mngr_lis_thread(self):
        if not isinstance(self, ParameterServer):
            self = self.local_value()
        msg = self.res_mngr_connm_lis.recv()
        while msg is not None:
            cmd, data = msg
            if cmd == "UNSET_DLY":
                self.is_wrk_delayed = False
                self.throttled_delay = -1
            elif cmd == "DELAY_WRK":
                self.is_wrk_delayed = True
                if data > 0:
                    self.throttled_delay = data
                else:
                    self.throttled_delay = -1
            msg = self.res_mngr_connm_lis.recv()
        self.res_mngr_connm_lis.close()

    # res_dict
    def tester_notify_stop(self):
        if not isinstance(self, ParameterServer):
            self = self.local_value()
        self.res_mngr_connm.send(("END_JOB", None))
    ...
    def determine_hybrid_sync_mode(self, pred_iterts=None, use_ssgd=False):
        if use_ssgd:
            pass
        else:
            wrk_iterts = copy.copy(pred_iterts)
            ...
            if self.use_heuristic:
                ...
                Tk = 0
                max_t = pred_iterts[-1]
                min_t = 1 / sum(list(map(lambda x: 1 / x, pred_iterts)))
                t_0 = min_t
                for i in range(1, self.worker_num):
                    t_1 = min_t + (max_t - min_t) * (pred_iterts[i] - pred_iterts[0]) / (max_t - pred_iterts[0])
                    T0 = t_0 * self.pgns.get_iter_num(i * self.batch_size, epoch)
                    T1 = t_1 * self.pgns.get_iter_num((i + 1) * self.batch_size, epoch)
                    if T0 < T1:
                        k = i
                        Tk = T0
                        break
                if k == 0:
                    k = self.worker_num
                    Tk = self.pgns.get_iter_num(self.worker_num * self.batch_size, epoch) * max_t
                # already got k

                # get dynamic k
                # group similar iter times of workers
                cluster = AgglomerativeClustering(distance_threshold=50, n_clusters=None).fit(wrk_iterts)  # 50ms threshold
                ...
                for i in range(len(clabels)):
                    lb = clabels[i]
                    itert = wrk_iterts[i]
                    groups[lb].append([i, itert])
                for _, itm in groups.items():
                    itm.sort(key=lambda x: x[1])
                # calculate dynamic k's time
                denom_sum = 0
                for _, itm in groups.items():
                    denom_sum += 1 / (self.pgns.get_iter_num(len(itm) * self.batch_size, epoch) * itm[-1][0])
                    # g_max_t.append([itm[-1][1], len(itm)])
                Tdyn = 1 / denom_sum
                if Tdyn < Tk:  # use dynamic k
                    pass  # use k
                    self.set_hybrid_sync_mode(k)
                else:  # use static k
                    pass  # use groups {gid: [[wid, itert], []]}
                    self.set_hybrid_sync_mode(groups=groups)

            # use ml
            inp = torch.tensor(wrk_iterts, dtype=torch.float32)
            out = self.mode_determ_m(inp)
            option = out[0]
            if option < 0.5:
                k = int(float(out[1]) + 0.5)  # use k
                self.set_hybrid_sync_mode(k)
            else:
                cluster = AgglomerativeClustering(distance_threshold=50, n_clusters=None).fit(wrk_iterts)  # 50ms threshold
                clabels = cluster.labels_
    ...
    # combined_strg_pred
    def set_curr_avail_res(self, wrk_id, ava_cpu, ava_bw):
        if not isinstance(self, ParameterServer):
            self = self.local_value()
        ...
        with self.next_iter_time_lock:
            ...
            self.ava_res_records[wrk_id] = self.ava_res_records[wrk_id][1:]
            self.ava_res_records[wrk_id].append([ava_cpu, ava_bw])

            # do straggler checking here
            # 1 check if heuristic needs to start
            # 2 check if straggler pred needs to start
            if self.strg_pred_remained_counts[wrk_id] == 0:
                self.strg_pred_remained_counts = [self.num_strg_pred_iter for _ in range(self.worker_num)]
                # do strg pred
                pred_result_res = []
                pred_result_itert = []
                res_tensor = torch.tensor(self.ava_res_records, dtype=torch.float32)
                for i in range(self.num_strg_pred_iter):
                    result = self.lstm_regr_m(res_tensor)
                    pred_result_res.append(result[:, -1:, :2])
                    pred_result_itert.append(result[:, -1:, 2:])
                    res_tensor = torch.cat([res_tensor[:, 1:, :], pred_result_res[-1]], dim=1)
                self.strg_pred_iterts = torch.cat(pred_result_itert, dim=1)
                # analyze straggler
                # max_t = float("-inf")
                for i in range(self.num_strg_pred_iter):
                    min_t = float("inf")
                    for w_id in range(self.worker_num):
                        w_t = self.strg_pred_iterts[w_id][i][0]
                        # if w_t > max_t:
                        #     max_t = w_t
                        if w_t < min_t:
                            min_t = w_t
                    for w_id in range(self.worker_num):
                        w_t = self.strg_pred_iterts[w_id][i][0]
                        ratio = (w_t - min_t) / min_t
                        if ratio > 0.2:
                            self.strg_tags[w_id][i] = True
                        else:
                            self.strg_tags[w_id][i] = False
                self.strg_tag_indices = [0 for _ in range(self.worker_num)]
                # # do filling out strg_exist_tags
                # for i in range(self.num_strg_pred_iter):
                #     exist = False
                #     for w_id in range(self.worker_num):
                #         tag = self.strg_tags[w_id][i]
                #         if tag is True:
                #             exist = True
                #             break
                #     if exist:
                #         self.strg_exist_tags[i] = True
                #     else:
                #         self.strg_exist_tags[i] = False
            self.strg_pred_remained_counts[wrk_id] -= 1

            # upload iter time info
            # use past iteration time
            if self.step_num > self.worker_num * self.num_strg_pred_iter:
                self.res_mngr_connm.send(("ITER_TIME", self.curr_iter_times))
            else:
                # use next wrk iter times
                payload = {}
                for i in range(self.worker_num):
                    index = self.strg_tag_indices[i]
                    payload[i] = self.strg_pred_iterts[i][index]
                self.res_mngr_connm.send(("ITER_TIME", payload))
            self.res_mngr_connm.send(("STEP_NUM", self.step_num))

            # sync mode determination
            state = False
            for w_id in range(self.worker_num):
                index = self.strg_tag_indices[w_id]
                tag = self.strg_tags[w_id][index]
                if tag:
                    state = True
                    break
            if state is self.curr_strg_state:
                pass  # keep the current state
            else:
                if state:  # strg occurs, do mode determination
                    iterts = []
                    for w_id in range(self.worker_num):
                        index = self.strg_tag_indices[w_id]
                        wt = self.strg_pred_iterts[w_id][index][0]
                        wt = float(wt)
                        iterts.append(wt)
                    self.determine_hybrid_sync_mode(pred_iterts=iterts)
                else:  # no strg, use SSGD
                    self.determine_hybrid_sync_mode(use_ssgd=True)
                self.curr_strg_state = state
            self.strg_tag_indices[wrk_id] += 1

            # if self.mode_determine_remained_counts[wrk_id] == 0:
            #     pass
            # self.mode_determine_remained_counts[wrk_id] -= 1

            # for e in self.next_iter_times:
            #     if e is None:
            #         # res_dist
            #         # use current wrk iter times
            #         self.res_mngr_connm.send(("ITER_TIME", self.curr_iter_times))
            ...
            # payload = {}
            # for i in range(self.worker_num):
            #     payload[i] = self.next_iter_times[i]
            # self.res_mngr_connm.send(("ITER_TIME", payload))
    ...
    def set_hybrid_sync_mode(self, k=-1, groups=None):
        if not isinstance(self, ParameterServer):
            self = self.local_value()

        if self.sync_hybrid_mode != k:
            if k != -1:
                if k == self.worker_num:
                    self.res_mngr_connm.send(("SSGD", None))
                else:
                    if self.is_wrk_delayed:
                        return
                    ssgd_delay = 0
                    arr: list = []
                    for w_id in range(self.worker_num):
                    ...
                    min_t = arr[-1]
                    for e in arr:
                        if (e - min_t) / min_t <= 0.2:
                            ssgd_delay = max_t - e
                            break
                    self.res_mngr_connm.send(("IF_ASGD", ssgd_delay))
                    decision = self.res_mngr_connm.recv()
                    if decision is None or decision == "SSGD":
                        return

                self.sync_hybrid_mode = k
                self.sync_hybrid_mode_groups = None
            elif groups is not None:
                self.sync_hybrid_mode = -1
                self.sync_hybrid_mode_groups = groups
        ...
        if self.sync_mode != sync_mode:
            ...
            if sync_mode == 0:
                ...
                if self.is_wrk_delayed:
                    return
                ssgd_delay = 0
                if self.sorted_next_iter_times is None:
                    arr: list = copy.copy(self.curr_iter_times)
                    arr.sort(reverse=True)
                    max_t = arr[0]
                    min_t = arr[-1]
                    for e in arr:
                    ...
                    arr: list = copy.copy(self.next_iter_times)
                    arr.sort(reverse=True)
                    max_t = arr[0]
                    min_t = arr[-1]
                    for e in arr:
    ...
    def ps_computing(self, worker_id, epoch_iter_idx_count, gradients, wrk_cp_t, ps2wrk_out_cm_t, in_cm_t_0, new_step_num=1):
        ...
        with self.sync_lock:
            ...
            # res_dist
            self.curr_iter_times[worker_id] = ps2wrk_out_cm_t + wrk_cp_t + ps2wrk_out_cm_t
            ...
            # dynamic groups
            if self.sync_hybrid_mode == -1:
                groups: dict = self.sync_hybrid_mode_groups
                the_group = None
                to_break = False
                for gid, itm in groups.items():
                    for lst in itm:
                        if worker_id == lst[0]:
                            the_group = gid
                            to_break = True
                    ...
                    if to_break:
                        break
                if the_group not in self.group_gradients:
                    self.group_gradients[the_group] = [gradients, 1, torch.futures.Future()]
                else:
                    self.group_gradients[the_group][0] += gradients
                    self.group_gradients[the_group][1] += 1
                if self.group_gradients[the_group][1] == len(groups[the_group]):
                    self.group_gradients[the_group][0] = self.group_gradients[the_group][0] / self.worker_num
                    for param, grad in zip(self.ps_params, self.group_gradients[the_group][0]):
                        param.grad = grad
                    self.optimizer.step()
                    for param in self.ps_params:
                        param.grad = None
                    self.group_gradients.pop(the_group, None)
                    self.group_gradients[the_group][2].set_result((self.ps_params, time.time(), self.in_cm_t))
                    return self.group_gradients[the_group][2]
                else:
                    return self.group_gradients[the_group][2]
            ...
            if self.enable_tree is False:
                ...
                # ):
                if self.sync_hybrid_mode != -1 and self.sync_worker_count >= self.sync_hybrid_mode:
                    ps_cp_t_0 = time.time()

                    # if sync_mode == 0 or (sync_mode == 1 and self.sync_worker_count > 1):
                    for param in self.ps_params:
                        param.grad = param.grad / self.worker_num
                    ...
                    if sync_mode == 0:
                        if self.is_wrk_delayed:
                            iter_t_arr = None
                            if self.use_current_iter_times_to_delay_wrk or self.sorted_next_iter_times is None:
                                iter_t_arr = self.curr_iter_times
                            else:
                                iter_t_arr = self.next_iter_times
                            ...
                            for i in range(self.worker_num):
                                ...
                                d_t = max_t - iter_t
                                if d_t > 0:
                                    self.worker_rrefs[i].rpc_async().put_delay_into_queue(d_t / 1000)
                            # delay by throttling res
                            if self.throttled_delay > 0:
                                time.sleep(self.throttled_delay)
...
class Worker:
    def __init__(
        ...
        # res_dist
        self.delay_time_queue = queue.Queue()

    # res_dist
    def put_delay_into_queue(self, dly):
        if not isinstance(self, Worker):
            self = self.local_value()
        self.delay_time_queue.put(dly)
    ...
    def run_worker(self):
        ...
        for epoch_idx in range(self.epoch_num):
            ...
            for iter_idx, x_pack in enumerator:
                ...
                # res_dict delay wrk
                try:
                    d_t = self.delay_time_queue.get(block=False)
                    time.sleep(d_t)
                except queue.Empty:
                    pass
                ...
                """
                if self.iter_count >= 110 and (self.iter_count - 110) % 1000 == 0:
                    # if self.iter_count == 500:
                    self.train_sr_lstm_model("cpu")
                    self.train_sr_lstm_model("bw")
                if self.iter_count >= 200 and (self.iter_count - 200) % 3000 == 0:
    ...
    def test_model(self):
        ...
        while True:
            ...
            if time_1 - time_0 >= 40:
                ...
                if step_num >= 64000:
                    self.stop_flag = True
                    self.ps_rrefs[0].rpc_async().tester_notify_stop()
                    break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ps_server_id", type=int, default=0)
