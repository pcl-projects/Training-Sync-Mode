...
def run_remote_py(sockm, pyfname="training_sgd_.py", path_prefix="/home/ubuntu/sgd/", **kwargs):
    ...
    cmd = f"python3 {path_prefix}/{pyfname} --job_name={job_name} --model_name={model_name} --rpc_rank={rpc_rank} --ps_num={ps_num} --worker_num={worker_num} --rpc_master_addr={rpc_master_addr} --rpc_master_port={rpc_master_port} --epoch_num={epoch_num} --data_partitioned={data_partitioned} --gpu_ids={gpu_ids} --wrk_server_ids={wrk_server_ids} --wrk_pack_toggles={wrk_pack_toggles} --tree_groups={tree_groups} --learning_rate={lr}"
