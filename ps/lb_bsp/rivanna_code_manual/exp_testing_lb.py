...
file_path_prefix = "/home/qxc4fh/zeyu_workspace/lb_bsp_manual"
...
# classifier_net.load_state_dict(torch.load("Archive/classifier_net.pt"))
# classifier_net.eval()
...
class Scheduler(object):
    ...
    def get_state(self, job_id):
        ...
        # threading.Thread(
        #     target=self.make_decision, args=(job_id, model_name, job_placement, agent_connm, resource_info, logger)
        # ).start()
    ...
    def delete_jobs(self):
        file_read_status = f"{file_path_prefix}/complete_status_lb.csv"
    ...
    def add_jobs(self):
        ...
        for _ in range(self.max_jobs - len(self.current_jobs)):
            if self.waiting_jobs:
                ...
                command_ps = (
                    f"python {file_path_prefix}/cifar_lb.py"
                    + " --model="
                    + str(model_name)
                    + " --job_name="
                    + str(job_name)
                ...
                for rank in range(1, num_workers + 1):
                    ...
                    self.client_record[rank].sendall(str.encode(command_worker))
                    time.sleep(2)
