        ...
        with self.lock:
            ...
            if self.mode_s == 1:
                for p, g in zip(self.model.parameters(), grads):
                    p.grad += ((g) * 1/self.batch_update_size)
                self.global_it +=1
                if self.global_it == self.batch_update_size-2:
                    self.global_it =0
                    self.mode_s = 0
                    fut = self.future_model
                    return fut
                # self.lgc_count = self.lgc_count + 1
                # if self.lgc_count == 3:
                #     prev_grad = g
                #     break
            elif self.mode_s == 0:
            ...
            if sync_mode == "0"  and self.mode_s == 0:
                self.curr_update_size += 1
                # if self.curr_update_size == 3:
                #     return self.future_model
                # self.global_it +=1
            fut = self.future_model

            # if sync_mode == "0" and self.curr_update_size >=self.batch_update_size:
            #     self.curr_update_size = 0

            if (sync_mode == "1" or self.curr_update_size >= self.batch_update_size-2) and self.mode_s==0:
                # with torch.no_grad():
                # for p in self.model.parameters():
...
def run_worker(ps_rref, data_dir, batch_size, num_epochs, worker, job_name, model, gpu_id):
    ...
    if worker == "worker4":
        ...
        import csv
        file_common = './complete_status.csv'
        with open(file_common, 'a', newline='') as file:
            writer = csv.writer(file)
...
def report_info_ps_thread(ps: ParameterServer):
    connm = SocketMsger.tcp_connect("127.0.0.1", 55673)
